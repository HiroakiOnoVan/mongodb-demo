...
class RealmObject {
    ...
    linkingObjects(objectType, propertyName) {
        const { objectSchema: linkedObjectSchema, properties } = this[REALM$1].getClassHelpers(objectType);
        const tableRef = Helpers.getTable(this[REALM$1].internal, linkedObjectSchema.tableKey);
        const property = properties.get(propertyName);
        assert(linkedObjectSchema.name === property.objectType, () => `'${linkedObjectSchema.name}#${propertyName}' is not a relationship to '${this.objectSchema().name}'`);
        ...
        const tableView = this[INTERNAL$1].getBacklinkView(tableRef, columnKey);
...
function validateConfiguration(config) {
    ...
    const { path, schema, schemaVersion, inMemory, readOnly, fifoFilesFallbackPath, sync, openSyncedRealmLocally, shouldCompact, deleteRealmIfMigrationNeeded, disableFormatUpgrade, encryptionKey, onMigration, } = config;
    if (path !== undefined) {
